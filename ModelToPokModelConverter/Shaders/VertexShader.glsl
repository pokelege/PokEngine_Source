...
void main()
{
	...
	{
	if( blendingIndex.x >= 0) animatedPosition += blendingWeight.x * animation[blendingIndex.x] * vec4(position, 1);
	if( blendingIndex.y >= 0) animatedPosition += blendingWeight.y * animation[blendingIndex.y] * vec4(position, 1);
	if( blendingIndex.z >= 0) animatedPosition += blendingWeight.z * animation[blendingIndex.z] * vec4(position, 1);
	if( blendingIndex.w >= 0) animatedPosition += blendingWeight.w * animation[blendingIndex.w] * vec4(position, 1);
	}
	else
	{
